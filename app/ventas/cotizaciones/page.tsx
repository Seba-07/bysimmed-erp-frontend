...
export default function Cotizaciones() {
  ...
  const loadData = async () => {
    try {
      const [cotRes, clientesRes, modelosRes, componentesRes, currencyRes] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData._id && productosSeleccionados.length === 0) {
      alert('Debes agregar al menos un producto a la cotización')
      return
    }

    const url = `${API_URL}/api/ventas/cotizaciones`
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/ventas/cotizaciones/${id}`, { method: 'DELETE' })
      if (res.ok) loadData()
    } catch (error) {
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="modal-actions-minimal">
                ...
                <button type="submit" className="btn-minimal btn-primary-minimal">
                  {formData._id ? 'Actualizar' : 'Crear'} Cotización
                </button>
              </div>
            </form>
          </div>
